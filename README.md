...
- [x] **gcp set up:**
    ...
    - [x] log into gcloud as an authorized user.  this step is necessary when setting up gcloud CLI on the raspberrypi, but may not be needed presently when setting up gcloud CLI on a laptop if the user already runs ```gcloud``` on this laptop.   If taking this step, follow instructions by copying the link provided into a browser, selecting login email from the popup, and copying the code back into the terminal window as shown. 
